...
function isGridActive() { return state.gridSessions.length >= 2; }
...
async function mountGridController(gs, cell, idx) {
  ...
  const tp = TERM_PRESETS[wpSettings.termFontSize] || TERM_PRESETS.medium;
  gs.controller = createPtyTerminalController({
    session: gs.session,
    machine: gs.machine || "",
...
function showGridCellConflictOverlay(gs) {
  ...
  removeGridCellConflictOverlay(gs);
  const overlay = createConflictOverlay("Active on another device", "Take Control", (e) => {
    e.stopPropagation();
...
function returnToTerminalView() {
  ...
  if (isDesktop()) {
    ...
    if (!state.desktopController) initDesktopTerminal();
  } else {
    resizePane().then(() => startPolling());
  }
  return true;
}
...
function backFromSettings() {
  ...
  showView(state.viewBeforeSettings || "sessions");
}
...
function addToGrid(session, machine) {
  ...
  if (isGridActive()) {
    ...
    state.gridSessions = [];
    openSession(session, machine || undefined);
  }
}
...
async function refreshRalphDetail() {
  ...
  try {
    const data = await api("/ralph", undefined, state.currentRalphMachine);
  ...
  try {
    const logData = await api("/ralph/log?project=" + encodeURIComponent(state.currentRalphProject), undefined, state.currentRalphMachine);
    if (logData.log != null) {
...
async function cancelRalph() {
  ...
  try {
    ...
    alert("failed to cancel: " + errorMessage(e));
  }
}
...
async function loadWorktreeBranches(project) {
  ...
  try {
    ...
    sel.innerHTML = branches.map(b => '<option value="' + escAttr(b) + '"' + (b === "main" ? " selected" : "") + '>' + escHtml(b) + '</option>').join("");
  } catch {
...
async function startRalph() {
  ...
  try {
    ...
    alert("failed to start: " + errorMessage(e));
  }
}

function restartRalph(planFile, agent, cleanup, auditFix) {
...
async function dismissRalph(project, event, machineUrl) {
  ...
  try {
    ...
    const data = await api("/ralph", undefined, machineUrl);
    ...
    if (sidebarRefreshTimer) { clearInterval(sidebarRefreshTimer); sidebarRefreshTimer = null; }
    state.lastSessionsHtml = "";
    await loadSessions();
    renderSidebar();
    startSidebarRefresh();
  } catch (e) {
    alert("failed to dismiss: " + errorMessage(e));
  }
}
    ...
    if (g.loops) {
      for (const loop of g.loops) {
        ...
        if ((prev === "running") && (cur === "done" || cur === "idle" || cur === "limit")) {
          const title = getMachines().length > 0 ? `${mName}: ralph` : "Wolfpack: ralph";
